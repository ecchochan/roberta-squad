...
def gen(paths):
    ...
    for i,context,qas in data_from_path(paths):
        context = translate_to_cn(context)
        
        for q in qas:
            q['question'] = translate_to_cn(q['question'].replace('<eop> ','\n'))
            if 'choices' in q:
                q['choices'] = [translate_to_cn(e) for e in q['choices']]
            if 'answer_text' in q:
                q['answer_text'] = translate_to_cn(q['answer_text'])
        for q in qas:
...
def evaluate(eval_dir):
  ...
  with torch.no_grad():
    for e, rs in tqdm(batches):
      ...
      for result, r in zip(zip(*(start_logits, end_logits)), rs):
        qid = r.qid
  ...
  for qid, predictions in tqdm(prediction_by_qid.items()):
    ...
    for result, r in predictions:
      ...
      if use_ans_class:
        ...
        cur_null_score = (start_top_log_probs[0] + end_top_log_probs[0]).tolist()
    ...
    for entry in nbest:
      ...
      if not best_non_null_entry:
        ...
        best_null_score = (entry.cur_null_score-(entry.start_log_prob + entry.end_log_prob)) if use_ans_class else -(entry.start_log_prob + entry.end_log_prob)
