...
import transformers
@register_model('roberta_qa_hf')
class RobertaQAModelHF(transformers.BertForQuestionAnswering, BaseFairseqModel):

    def __init__(self, *args, **kwargs):
        transformers.BertForQuestionAnswering.__init__(self, *args, **kwargs)
...
@register_model_architecture('roberta_qa_hf', 'roberta_qa_large_hf')
...
@register_criterion('squad2_hf')
class SQuAD2Criterion_hf(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        (total_loss, start_logits, end_logits) = model(tokens, start_positions=start_positions, end_positions=end_positions)
...
class SQuAD2Criterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        (start_logits, end_logits, cls_logits), extra = model(tokens, attention_mask=tokens.ne(1))
        start_logits = start_logits.squeeze(-1)
        end_logits = end_logits.squeeze(-1)
